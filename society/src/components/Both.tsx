...
interface TextInputProps {
  ...
  onEdit?: () => void;
  initialValue?: string;
  onEditSubmit?: () => void; // Added callback for when an edit is submitted
}
...
export const TextInput: React.FC<TextInputProps> = ({
  ...
  const handleSubmit = (e?: FormEvent<HTMLFormElement>) => {
    ...
    if (value.trim() && !isLoading) {
      // If this is being edited (was previously condensed)
      if (isCondensed && onEditSubmit) {
        onEditSubmit(); // Notify parent that an edit was submitted
      }
      onSubmit(value.trim());
    }
  };
